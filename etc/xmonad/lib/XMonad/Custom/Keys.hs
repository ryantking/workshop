...
myKeys = do
  ...
  return
    ...
    , ("M-b f"                   , spawn myFileManager)

    -- KB_GROUP Emacs
    , ("M-w w"                   , spawn myEmacs)
    , ("M-w c", spawn (myEmacs ++ "--eval '(ryan-org-capture-open-frame)'"))
    , ("M-w a", spawn (myEmacs ++ "--eval '(org-agenda nil \"d\")'"))
    , ("M-w b", spawn (myEmacs ++ "--eval '(ibuffer)'"))
    , ("M-w d", spawn (myEmacs ++ "--eval '(dired nil)'"))
    , ("M-w s", spawn (myEmacs ++ "--eval '(eshell)'"))

    -- KB_GROUP Run Scripts
    , ("M-p e", spawn "/home/ryan/.config/xmonad/bin/dm-emacs.sh")
    , ("M-p x", spawn "/home/ryan/.config/xmonad/bin/dm-xmonad.sh")

    -- KB_GROUP Scratchpads
    , ("M-s t", namedScratchpadAction myScratchPads "terminal")
    , ("M-s c", namedScratchpadAction myScratchPads "calculator")
    , ("M-s v", namedScratchpadAction myScratchPads "pavu")
    , ("M-s n", namedScratchpadAction myScratchPads "networkmanager")
    , ("M-s b", namedScratchpadAction myScratchPads "bluetooth")
    ...
    , ("M-S-f f f", spawn "flameshot full -p ~/Pictures/screenshots")
    ...
    , ("S-<XF86AudioRaiseVolume>", spawn "amixer set Master 10%+")
    , ("S-<XF86AudioLowerVolume>", spawn "amixer set Master 10%-")
    ]
    -- END_KEYS
