...
(defun vulpea-make-capture-target (key headline)
  "Return a target for a capture."
  (lambda ()
    (let ((note (org-capture-get key)))
...
(defun vulpea-capture-task-template ()
  ...
  (let ((note (vulpea-select
                ...
                (lambda (note)
                  (let ((tags (vulpea-note-tags note)))
                    (seq-contains-p tags "project"))))))
    (org-capture-put :task-project note)
    (if (vulpea-note-id note)
      "* TODO %? %^G%{extra}"
      "* TODO %? :REFILE: %^G%{extra}")))
