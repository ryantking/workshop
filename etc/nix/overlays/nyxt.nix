final: prev: {
  nyxt = prev.stdenv.mkDerivation rec {
    pname = "nyxt";
    inherit (final.lispPackages.nyxt.meta) version;
    src = final.lispPackages.nyxt;

    nativeBuildInputs = with prev; [wrapGAppsHook];
    ...
    binPath = prev.lib.optionals (!prev.stdenv.isDarwin) [prev.xclip];
      ...
      + (
        ...
        then ''
          ...
          wrapGApp $out/Applications/Nyxt.app/Contents/MacOS/nyxt \
            --prefix PATH : "${prev.lib.makeBinPath binPath}" \
            --argv0 nyxt
