...
(fn mod-run [mod-name func-name ...]
  `(let [mod# (require ,(tostring mod-name))
         func# (. mod# ,(tostring func-name))]
     (func# ,...)))

(fn with-setup-func [cfg mod-name func-name setup-opts]
  "Returns a plugin configuration that calls the given module's named setup
  function"
  `(let [cfg# ,cfg]
    (tset cfg# :config
         (fn []
           (let [mod# (require ,(tostring mod-name))
                 func# (. mod# ,(tostring func-name))]
             (func# ,(when setup-opts setup-opts)))))
    cfg#))

(fn with-setup [cfg mod-name setup-opts]
  "Returns a plugin configuration that calls the given module's setup function"
  `,(with-setup-func cfg mod-name :setup setup-opts))

{: augroup : autocmd : g : se- : mod-run : with-setup-func : with-setup}
